...
# 'table1[['time']]'                           confidence intervals and comp[['time']]
...
settings[['cv']] <- matrix(c(NA, NA), nrow = 1, ncol = 3)# Critical values as in eq. (41) and (47).
...
W_data <- D_matrix(:, 2:length(D_matrix))
...
for (row0 in 1:(dim(specs)[1])){

    Vbar <- specs[[row0]][[1]]
    cvalue <- specs[[row0]][[4]]

    if (strcmp(cvalue, 'SN2S')){
        Vbar <- c(paste0('$Bar{V}$ <- ', num2str(Vbar)))
        cvalue0 <- 'self-norm'
    } else if (strcmp(cvalue, 'EB2S')){
        Vbar <- "!"
        cvalue0 <- 'bootstrap'
    }
