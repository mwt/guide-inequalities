...
function salida = m_function(W_data, A_matrix, theta, J0_vec, Vbar, IV_matrix, grid0)
    ...
    for mm0 = 1:n
        ...
        if isempty(IV_matrix)
            ...
            Z_vec = ones(J0, 1);
            Z3_vec = 1 * (IV_matrix(1:J0, 2) > median(IV_matrix(:, 2))); % employment rate
            Z5_vec = 1 * (IV_matrix(1:J0, 3) > median(IV_matrix(:, 3))); % average income in market
            Z7_vec = 1 * (IV_matrix(1:J0, 4) > median(IV_matrix(:, 4))); % median income in market
