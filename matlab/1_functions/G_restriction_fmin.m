function [c,ceq] = G_restriction_fmin(W_data,Dist_data,A_matrix,theta0,J0_vec,Vbar,IV,grid0,test0,cvalue,alpha_input,num_boots,An_vec,hat_r_inf)
% T_n(theta) - cv(theta,1-alpha), % for CCK using SN2S or EB2s
% S_n(theta) - cv(theta,1-alpha), % for AK using SPUR1 or SB2S

if strcmp(test0,'CCK')
    X_data   = m_functionv2(W_data,Dist_data,A_matrix,theta0,J0_vec,Vbar,IV,grid0);
